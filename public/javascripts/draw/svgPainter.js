var Painter = function (svgId) {
    ...
    this.nowId = (Math.random()*10000000).toString(16).substr(0,4)+'-'+(new Date()).getTime()+'-'+Math.random().toString().substr(2,5); // id 累加器,为每个元素赋值唯一的 id

    this.color = '#000'; // 当前选择的颜色
    ...
    function unified(){
        ...
        if(self.shape == 'path' && self.color != "white"){
            $('.itemBox').css({
        ...
        }else if(self.shape != 'path' && self.color != "white"){
            $('.itemBox').css({
                "background-color" :'#f0f0f0'
            });
            $('#toolBox').parent().css({
    ...
    this.svg.addEventListener('mousedown', function (e) {
        ...
        unified();
        socket.emit('config', {"name":'init'});
    }, false);
    this.svg.addEventListener('touchstart', function (e) {
        self.moveStartX = e.touches[0].clientX;
        self.moveStartY = e.touches[0].clientY;
        if (self.target !== self.svg && e.target === self.target) {
            self.draging = true;
        } else {
            self.drawing = true;
            self.drawStartX = e.touches[0].clientX;
            self.drawStartY = e.touches[0].clientY;
        }
        unified();
        socket.emit('config', {"name":'init'});
    }, false);
    ...
    this.svg.addEventListener('touchmove', function (e) {
        if (Math.abs(e.touches[0].clientX - self.moveStartX) > 10 || Math.abs(e.touches[0].clientY - self.moveStartY) > 10) {
            if (self.draging) {
                self.moving = true;
            }
        }
        ...
        if (self.drawing) {
            e.target.style.cursor = 'crosshair';
            self.draw(self.drawStartX, self.drawStartY, e.touches[0].clientX, e.touches[0].clientY);
        }
    }, false);
...
Painter.prototype.fill = function (shape, attr, isHollow, id) {
    if (id != undefined) {
        ...
        id = this.nowId;
        this.nowId = (Math.random()*10000000).toString(16).substr(0,4)+'-'+(new Date()).getTime()+'-'+Math.random().toString().substr(2,5);
    }
